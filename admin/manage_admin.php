...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'create') {
            ...
            $ins->execute([':name' => $name, ':email' => $email, ':role' => 'admin', ':hash' => $hash]);
            set_flash('success', 'Admin user created successfully.');
            header('Location: ' . url_for('/admin/manage_admin.php'));
            exit;
            ...
            $upd = $pdo->prepare('UPDATE users SET name = :name, email = :email WHERE id = :id AND role = "admin"');
            $upd->execute([':name' => $name, ':email' => $email, ':id' => $id]);
            set_flash('success', 'Admin user updated successfully.');
            header('Location: ' . url_for('/admin/manage_admin.php'));
            exit;
            ...
            $hash = password_hash($password, PASSWORD_DEFAULT);
            $upd = $pdo->prepare('UPDATE users SET password_hash = :hash WHERE id = :id AND role = "admin"');
  ...
  <header class="bg-blue-600 text-white">
    <div class="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
      <div class="flex items-center gap-2">
        ...
        <h1 class="text-lg font-semibold">Admin — Manage Admin Users</h1>
      </div>
    ...
    <div class="grid grid-cols-12 gap-6">
      ...
      <section class="col-span-12 md:col-span-9">
        <div class="rounded-lg border bg-white p-6 shadow-sm min-h-[70vh]">
          <h2 class="text-base font-semibold mb-3">Manage Admin Users</h2>
          <p class="text-sm text-gray-600 mb-4">Create, update, reset passwords, and remove Admin accounts.</p>
          <?php if ($flash): ?>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div class="rounded border p-4">
              <h3 class="text-sm font-semibold mb-2">Create Admin User</h3>
              <form method="POST" action="<?= htmlspecialchars(url_for('/admin/manage_admin.php')) ?>" class="space-y-2 text-sm">
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
              ...
              <tbody>
                <?php if (!$adminUsers): ?>
                  <tr>
                    <td colspan="4" class="px-3 py-4 text-center text-gray-500">No Admin users found.</td>
                  </tr>
                <?php else: ?>
                  <?php foreach ($adminUsers as $u): ?>
                    <tr class="border-b">
                      <td class="px-3 py-2">
                        <form method="POST" action="<?= htmlspecialchars(url_for('/admin/manage_admin.php')) ?>" class="flex items-center gap-2">
                      ...
                      <td class="px-3 py-2">
                        <div class="flex items-center gap-2">
                          ...
                          <form method="POST" action="<?= htmlspecialchars(url_for('/admin/manage_admin.php')) ?>" onsubmit="return confirm('Delete this Admin user? This action cannot be undone.');">
